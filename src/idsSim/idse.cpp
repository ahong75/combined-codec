...
/*  Creates vector of oligo number and respective oligo string from given input
   fafsta file Note: Input file should have oligos in increasing order
*/
...
/* Generates insertion, deletion, substitution, or no error depending on percent
   weights of each specified by the user (in json)
*/
int generateIdsType() {
  double rnd = (double)randomIdsDist(generator) / 100.0;
  // Map of indelsub error percentenages
  map<int, float> errorWeights = {{0, parser->insertErrorRate},
                                  {1, parser->deleteErrorRate},
                                  {2, parser->substitutionErrorRate},
                                  {3, 1 - parser->insertErrorRate -
                                          parser->deleteErrorRate -
                                          parser->substitutionErrorRate}};
  // Insert error
  ...
  // Substitution Error
  else if (rnd > errorWeights[0] + errorWeights[1] &&
           rnd <= errorWeights[0] + errorWeights[1] + errorWeights[2]) {
    return 2;
  }
  // No Error
  return 3;
...
/* Performs insertion, deletion, and substitution on an oligo string (applied to
 * each base)
 */
...
/* Creates orderedMap and unordered output files of oligo number and respective
 * oligo string
 */
void generateOrderedOutputFile(vector<string> oligos) {
  ...
  for (int i = 0; i < parser->numberReads; i++) {
    // Randomly select oligo from oligos vector (akin to a sequencer selecing a
    // random oligo in a DNA pool)
  ...
  std::vector<std::pair<int, string>> elems(orderedMap.begin(),
                                            orderedMap.end());
...
void generateUnorderedOutputFile(vector<string> oligos) {
  ...
  for (int i = 1; i < parser->numberSamples + 1; i++) {
    ...
    unorderedOutFile.open(
        (parser->unorderedOutput)
                .substr(0, (parser->unorderedOutput).size() - 7) +
            to_string(i) + ".fafsta",
        ios::out | ios::trunc);
    for (int i = 0; i < parser->numberReads; i++) {
      // Randomly select oligo from oligos vector (akin to a sequencer selecing
      // a random oligo in a DNA pool)
      int oligoNumber = randomOligo(generator);
      // Get string with indelsub errors
      string errorStr = performIds(oligos[oligoNumber]);
